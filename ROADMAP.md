...
| Clipboard | `PySide6.QtGui.QGuiApplication.clipboard()` | Standard `QClipboard`, same API C++/Qt would use |
...
### Phase 0 — Formalize existing services
...
clipboard via `QClipboard`, calls the core, prints/returns the result.

**Watch for**: on X11/Wayland the clipboard owner is the process itself —
if the script exits immediately after `setText()`, the content can be lost
unless something (e.g. Klipper) has already grabbed it. Verify this early;
mitigate with a short delay/`processEvents()` before exit if needed.

**Deliverable**: the CLI functionally replaces the bash script for all 3
services.

### Phase 2 — Result Inspector (PySide6)
...
### Phase 3.5 — Optional: Kirigami/QML picker
Only if the Widgets picker doesn't feel polished enough. PySide6 can load
Kirigami QML components via `QQmlApplicationEngine` (`org.kde.kirigami`,
already present as part of the KDE runtime) — no C++, no KF6 bindings.
Low-risk: touches only the picker window, not the engine or review box.
...
D-Bus instead of a focus-stealing popup.

### Phase 5 — Optional: background daemon
...
already-running process instead of a fresh interpreter each time.

## Beyond the MVP

Once Phase 3 is working, growing the service catalog is just adding YAML
files — no engine changes. Pull from `ai-microservices.tables.md`
opportunistically, cheapest/most useful first:

- Cheap, high value: summarize, make-concise, academic-style-convert,
  explain-code, review-code.
- Genuinely separate mini-projects (not "just a YAML file"): anything
  needing external tools/APIs (citation verifier via Crossref/OpenAlex,
  literature screening, research preflight). Attempt these only once the
  core engine has been stable for a while.

There is no fixed target count — `ai-microservices.tables.md` is a backlog,
not a deliverable.
